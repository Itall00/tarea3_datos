...
ggplot(etr_mean, aes(x = fecha, y = ET, color = nombre)) +
  ...
  labs(
    x = "AÃ±o", y = "Etr (mm)", title = "Consumo medio por cobertura de suelo",
    color = "Cobertura"
  )
...
ggplot(etr_total, aes(x = fecha, y = ET, color = nombre)) +
  geom_line(linewidth = 1) +
  labs(
    x = "AÃ±o", y = "Etr (mm)", title = "Consumo total por cobertura de suelo",
    color = "Cobertura"
  )
...
et.mod <- project(et.mod, crs(km))
st_crs(km)==st_crs(et.mod)
...
et.y <- project(et.y, crs(km))
st_crs(km)==st_crs(et.y)
...
cuenca.t = read_sf(paste0(path, "/cuenca.kml")) %>%
  # reproyectar a coordenadas geogrÃ¡ficas
  st_transform(4326)
######## Modelo de ElevaciÃƒÂ³n Digital (DEM) ---------------------------------------
...
# crear matriz de reclasificaciÃƒÂ³n
rcl.matrix = matrix(
  c(-1, 250,1,
    250,Inf,2),
  ncol = 3, byrow = TRUE)
...
# Mejorar resultado cambiando el tamaÃ±o de la ventana movil
...
plot(aspect, main = "ExposiciÃƒÂ³n")
hist(aspect)
...
#graficar
plot(aspect.rec, col = colores, main = "Exposición reclasificada")
...
####### Exportar derivadas topogrÃ¡ficas -----------------------------------------
# nombre del archivo
...
fechas.et = names(et) %>% 
  ...
  str_sub(start = 26, end = 32) %>%
  # le damos formato de fecha, %Y = YYYY, %j=ddd (numero de dia del aÃ±o o dia juliano)
  as.Date("%Y%j")
fechas.et
...
plot(lc.orig, main = "LC sin matorrales", col=colores)
...
# Modificar EvapotranspiraciÃƒÂ³n real ---------------------------------------
...
# Transformar valores de mm a m3/aÃƒÂ±o
...
ggplot(etrmod_total, aes(x = fecha, y = ET, color = name))+
  geom_line(linewidth = 1)+
  labs(x = 'AÃƒÂ±o', y = 'Etr (m3/aÃƒÂ±o)', title = 'Consumo total por cobertura de suelo',
       color = 'Cobertura')
...
etrmod_total %>% ggplot(aes(x = '', y = prop, fill = name))+
  ...
  facet_wrap(.~fecha, drop = TRUE)+ #crea un grafico por cada valor unico de fecha
  scale_fill_brewer(palette="Set1")+ # remove background, grid, numeric labels
  labs(fill = 'Cobertura', title = 'EvapotranspiraciÃƒÂ³n real anual por cada cobertura de suelo')

# Graficar solo un aÃƒÂ±o
etrmod_total %>% 
...
plot(cuenca.t)
# Etr anual de la cuenca modificada para el balance hidrico
et.mod <- project(et.mod, crs(cuenca.t))
st_crs(cuenca.t)==st_crs(et.mod)
...
# Etr anual de la cuenca riginalpara el balance hidrico
et.y <- project(et.y, crs(cuenca.t))
st_crs(cuenca.t)==st_crs(et.y)
...
ggplot(data.year)+
  geom_line(aes(x = fecha, y = pp, color = "PrecipitaciÃƒÂ³n"), linewidth = 0.8)+
  ...
  labs(x = "tiempo", y = "(mm)", title = "Serie de tiempo mensual de componentes del BH",
       subtitle = "Los aÃƒÂ±os sin medicion de caudal, faltan datos en algunos meses",
       color = "")
